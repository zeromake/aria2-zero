...
public:
  void setUp()
  {
    ...
    registry_ = aria2::make_unique<ExtensionMessageRegistry>();
    dispatcher_ = aria2::make_unique<MockBtMessageDispatcher>();
    messageFactory_ = aria2::make_unique<MockBtMessageFactory>();
    dctx_ = std::make_shared<DownloadContext>();
    auto option = std::make_shared<Option>();
    requestGroup_ = aria2::make_unique<RequestGroup>(GroupId::create(), option);
    requestGroup_->setDownloadContext(dctx_);

    factory_ = aria2::make_unique<DefaultExtensionMessageFactory>();
